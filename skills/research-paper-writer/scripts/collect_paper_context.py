...
def iter_files(root: Path):
    for path in root.rglob("*"):
        ...
        if "skills/research-report-writer" in normalized or "skills/research-paper-writer" in normalized:
            continue
        if not path.is_file():
            continue
...
def classify(path: Path) -> str:
    ...
    if path.suffix.lower() in {".png", ".jpg", ".jpeg", ".svg"}:
        return "figure-asset"
    if path.suffix.lower() in {".xlsx", ".xls", ".parquet", ".npy", ".npz", ".mat", ".h5", ".hdf5"}:
        return "data-asset"
    if "/scripts/" in text:
        return "script"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Collect likely paper-writing context files.")
    ...
    print(f"# Paper Context Candidates\n")
    print(f"- root: `{root}`")
    ...
    for kind in ("doc-output", "latex-output", "report", "artifact", "figure-asset", "data-asset", "bibliography", "log", "script", "design-doc", "other"):
